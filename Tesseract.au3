...
#cs
	...
	Changelog:		---------15/02/09---------- v0.1
					...
					---------06/11/16-------- v0.7e
					Mods by Erik Gregg begin here
					Chaged temp path to Windows Temp by default
					Added configuration variable for the executable location

#ce
#EndRegion Header
#Region Global Variables and Constants
Global $last_capture
Global $tesseract_temp_path = "C:\Windows\Temp"
Global $g_tesseract_exe = _ProgramFilesDir() & "\tesseract-ocr\tesseract.exe"
...
; #FUNCTION# ;===============================================================================
;
; Name...........:	_TesseractExePathSet()
; Description ...:	Sets the path to the Tesseract executable.
;						The default location is _ProgramFilesDir() & "\tesseract-ocr\tesseract.exe".
; Syntax.........:	_TesseractExePathSet($exe_path)
; Parameters ....:	$exe_path	- The path to the executable.
; Return values .: 	On Success	- Returns 1.
;                 	On Failure	- Returns 0.
; Author ........:	Erik Gregg
; Modified.......:
...
;
; ;==========================================================================================
func _TesseractExePathSet($exe_path)

	$g_tesseract_exe = $exe_path
...
func _TesseractScreenCapture($get_last_capture = 0, $delimiter = "", $cleanup = 1, $scale = 2, $left_indent = 0, $top_indent = 0, $right_indent = 0, $bottom_indent = 0, $show_capture = 0)
	...
	ConsoleWrite("T::: Capturing to " & $capture_filename & @LF)

	CaptureToTIFF("", "", "", $capture_filename, $scale, $left_indent, $top_indent, $right_indent, $bottom_indent)

	Local $iReturn = ShellExecuteWait($g_tesseract_exe, $capture_filename & " " & $ocr_filename, "C:\")
	ConsoleWrite("T::: Tesseract returned " & $iReturn & @LF)
...
func _TesseractWinCapture($win_title, $win_text = "", $get_last_capture = 0, $delimiter = "", $cleanup = 1, $scale = 2, $left_indent = 0, $top_indent = 0, $right_indent = 0, $bottom_indent = 0, $show_capture = 0)
	...
	Local $tInfo
	dim $aArray, $final_ocr, $xyPos_old = -1, $capture_scale = 3
	Local $tSCROLLINFO = DllStructCreate($tagSCROLLINFO)
	DllStructSetData($tSCROLLINFO, "cbSize", DllStructGetSize($tSCROLLINFO))
	DllStructSetData($tSCROLLINFO, "fMask", $SIF_ALL)
	...
	Local $cmd = $capture_filename & " " & $ocr_filename & " tsv"
	ConsoleWrite("T::: Tesseract cmd: " & $g_tesseract_exe & " " & $cmd & @LF)
	Local $iReturn = ShellExecuteWait($g_tesseract_exe, $cmd, "C:\")
	ConsoleWrite("T::: Tesseract returned " & $iReturn & @LF)
	; If no delimiter specified, then return a string
	if StringCompare($delimiter, "") = 0 Then
		$final_ocr = FileRead($ocr_filename_and_ext)
	Else
		; Read TSV output into an array and return it
		Local $aArray = _ParseCSV($ocr_filename_and_ext,Chr(9),"Couldn't open TSV",True)
		; _ArrayDelete($aArray, 0)
		; Append the recognised text to a final array
		$final_ocr = $aArray
	EndIf
...
Func _ParseCSV($f,$Dchar,$error,$skip)
  ...
  While 1
       ...
       $row_array = StringSplit($line,$Dchar)
        ...
        If $row_size <> UBound($row_array) Then  ConsoleWrite("Row: " & $i & " has different size ")
